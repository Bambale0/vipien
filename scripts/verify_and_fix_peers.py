...
def get_awg_allowed_ips():
    proc = subprocess.run(
        ["sudo", "awg", "show", "wg0", "allowed-ips"], capture_output=True, text=True
    )
...
def main():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT public_key, preshared_key, address, config_name FROM configs WHERE is_active = 1"
    )
    rows = cur.fetchall()
    ...
    for pub, psk, addr, cfg in rows:
        ...
        if (pub in allowed) or (f"{addr}/32" in allowed):
            ...
            if ok and (pub in allowed or f"{addr}/32" in allowed):
                ...
                peer_block = f"\n[Peer]\nPublicKey = {pub}\nPresharedKey = {psk}\nAllowedIPs = {addr}/32\n"
                p = subprocess.run(
                    ["sudo", "tee", "-a", "/etc/amnezia/amneziawg/wg0.conf"],
                    input=peer_block,
                    text=True,
                    capture_output=True,
                )
