...
def db_duplicates():
    ...
    try:
        ...
        cur=conn.cursor()
        cur.execute("SELECT public_key, COUNT(*), GROUP_CONCAT(config_name) FROM configs WHERE public_key IS NOT NULL AND public_key!='' GROUP BY public_key HAVING COUNT(*)>1")
        rows=cur.fetchall()
        out.append(('public_key_duplicates', rows))
        cur.execute("SELECT preshared_key, COUNT(*), GROUP_CONCAT(config_name) FROM configs WHERE preshared_key IS NOT NULL AND preshared_key!='' GROUP BY preshared_key HAVING COUNT(*)>1")
        rows2=cur.fetchall()
...
def awg_duplicates():
    ...
    duplicates=[(ip, len(pubs), pubs) for ip,pubs in mapping.items() if len(pubs)>1]
    return duplicates
...
def run_capture():
    ...
    try:
        subprocess.run(['sudo','timeout', str(CAPTURE_SECONDS), 'tcpdump', '-i', 'any', '-n', 'udp', 'port', '51820', '-w', PCAP_PATH], check=False)
        return ('ok', PCAP_PATH)
    except Exception as e:
...
def analyze_pcap():
    ...
    samples=[]
    try:
        proc=subprocess.run(['sudo','tcpdump','-nn','-r', PCAP_PATH], capture_output=True, text=True)
        txt=proc.stdout or ''
