...
class WireGuardManager:
    ...
    def generate_keypair(self) -> Tuple[str, str]:
        ...
        try:
            ...
            private_key = subprocess.check_output(['sudo', 'wg', 'genkey']).decode().strip()
        ...
        try:
            ...
            for _ in range(10):  # Попробуем 10 случайных IP
                ...
                try:
                    conn = sqlite3.connect('wg_bot.db')
                    cursor = conn.cursor()
                    cursor.execute('SELECT COUNT(*) FROM configs WHERE address = ?', (ip,))
...
"""
    ...
    def add_peer_to_amnezia_awg(self, public_key: str, preshared_key: str, allowed_ip: str):
        ...
        try:
            ...
            with self._config_lock():
                ...
                self._add_peer_internal(public_key, preshared_key, allowed_ip, max_retries, sleep_base)
            return
        except Exception as e:
            self.logger.error(f"Unexpected error adding peer to awg (outer): {e}", exc_info=True)
            try:
    ...
    def _add_peer_internal(self, public_key: str, preshared_key: str, allowed_ip: str, max_retries: int, sleep_base: int):
        psk_file = None
        try:
            ...
            if peer_exists:
                try:
                    ...
                    self.logger.info(f"Removed existing peer {public_key[:8]}... before re-adding")
                except Exception as e:
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    if proc.returncode == 0:
                        self.logger.info(f"Added peer {public_key[:8]}... to awg with IP {allowed_ip} (attempt {attempt})")
                        added = True
                        break
                    else:
                        self.logger.warning(f"awg set failed (attempt {attempt}): {proc.stderr.strip()}")
                except Exception as e:
                    self.logger.warning(f"Exception running awg set (attempt {attempt}): {e}")

                time.sleep(sleep_base * attempt)

            # Если добавление через awg не сработало, всё равно добавляем запись в конфиг сервера
            if not added:
                self.logger.warning("Adding peer via awg failed after retries — appending to server config")
                self._save_peer_to_config_file(public_key, preshared_key, allowed_ip)
            ...
            try:
                ...
                if public_key in out or f"{allowed_ip}/32" in out:
                    self.logger.info(f"Verified peer present in awg allowed-ips: {public_key[:8]} -> {allowed_ip}")
                else:
                    # one more attempt to append config and reload
                    self.logger.warning("Peer not visible in allowed-ips after add — appending and retrying awg set once more")
                    self._save_peer_to_config_file(public_key, preshared_key, allowed_ip)
                    # try one more awg set
                    try:
                        proc = subprocess.run(cmd, capture_output=True, text=True)
                        if proc.returncode == 0:
                            self.logger.info("awg set succeeded on retry after appending to config")
                    except Exception:
                        pass
                    ...
                    if public_key in out2 or f"{allowed_ip}/32" in out2:
                        self.logger.info(f"Peer visible after fallback append: {public_key[:8]} -> {allowed_ip}")
                    else:
                        self.logger.error(f"Peer still not visible after fallback: {public_key[:8]} -> {allowed_ip}")
            except Exception as e:
                self.logger.debug(f"Failed to verify awg allowed-ips: {e}")

        except Exception as e:
            self.logger.error(f"Unexpected error adding peer to awg (inner): {e}", exc_info=True)
        finally:
    ...
    def _save_peer_to_config_file(self, public_key: str, preshared_key: str, allowed_ip: str):
        ...
        try:
            ...
            try:
                read_proc = subprocess.run(['sudo', 'cat', server_config_path], capture_output=True, text=True)
                if read_proc.returncode == 0:
                    config_content = read_proc.stdout
                else:
                    self.logger.warning(f"Unable to read {server_config_path} via sudo: {read_proc.stderr.strip()}")
            ...
            if f"PublicKey = {public_key}" in config_content:
                self.logger.info(f"Peer {public_key[:8]}... already exists in awg config")
                return
...
"""
            ...
            try:
                ...
                subprocess.run(['sudo', 'cp', server_config_path, backup_path], check=False)
                ...
                proc = subprocess.run(['sudo', 'bash', '-c', mv_cmd], capture_output=True, text=True)
                if proc.returncode == 0:
                    self.logger.info(f"Atomically replaced {server_config_path} and created backup {backup_path}")
                else:
                    self.logger.error(f"Failed to atomically replace server config: {proc.stderr}")
                    # Попытка append через tee как fallback
                    proc2 = subprocess.run(['sudo', 'tee', '-a', server_config_path], input=peer_config, text=True, capture_output=True)
                    if proc2.returncode == 0:
                        self.logger.info(f"Appended peer {public_key[:8]}... to {server_config_path} via tee fallback")
                    else:
                        self.logger.error(f"Fallback append also failed: {proc2.stderr}")
            except Exception as e:
